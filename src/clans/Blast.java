...
public class Blast {
    ...
    public Blast(StringBuffer errbuff,boolean addblastvbparam) {
    ...
    public MinimalHsp[] gethits(AminoAcidSequence[] oldaln, MinimalHsp[] oldblasthits, AminoAcidSequence[] newaln,
        ...
        try{
            ...
            ThreadedStreamReader errread;
            ThreadedStreamReader inread;
            Runtime rt=Runtime.getRuntime();
            try{
                Process p=rt.exec(formatcommand);
                errread=new ThreadedStreamReader(new BufferedReader(new InputStreamReader(p.getErrorStream())),errout);
                inread=new ThreadedStreamReader(new BufferedReader(new InputStreamReader(p.getInputStream())),inout);
            ...
            while (allstart<seqnum){
                ...
                FileHandling.saveblastappend(outwrite,mythreads[freethread].retarr,nameshash.get(mythreads[freethread].query.name).intValue());
            ...
            if(exhaustive==1){
                ...
                retarr=FileHandling2.blast("tmpblasthsp.txt",cutoff);
    ...
    public MinimalHsp[] gethits(AminoAcidSequence[] inarr, String cmd, String formatdbpath, String blastpath, int cpu,
        ...
        if(tmpfile.canRead()&&readblast){
            ...
            MinimalHsp[] retarr=FileHandling2.blast("tmpblasthsp.txt",pval);
            return retarr;
        }else{
            try{
                PrintWriter outwrite=new PrintWriter(new BufferedWriter(new FileWriter("tmpblasthsp.txt")));
    ...
    public MinimalHsp[] gethits(MinimalHsp[] blasthits, AminoAcidSequence[] inarr, String cmd, String formatdbpath,
        ...
        try{
            ...
            System.out.println("re-reading hsp's");
            blasthits=FileHandling2.blast("tmpblasthsp.txt",pval);
            //}
            //all threads started and done. the values are stored in retarr.
            try{
                //delete the blast database and the files that formatdb created and the blast query files
...
class blastthread extends java.lang.Thread{
     ...
     * @param lowmem
     */
    public blastthread(Blast parent, int allseqnum, String cmd, String blastpath, String blastdbname,
            AminoAcidSequence query, double eval, double pval, float coverage, float scval, float ident,
            Vector<HighScoringSegmentPair>[] retvecarr, HashMap<String, Integer> nameshash, int verbose, Runtime rt, int threadnum,
    ...
    int verbose;
    int allseqnum=0;
    Vector<HighScoringSegmentPair>[] retarr;
    ...
    public void run(){
        ...
        if((referencedb.length>0)&&(blastpath.indexOf("blastpgp")>-1)){
            ...
            try{
                ...
                BufferedReader perr;
                BufferedReader pin;
                ThreadedStreamReader perrread;
                ThreadedStreamReader pinread;
                ...
                perrread=new ThreadedStreamReader(perr,errout);
                pinread=new ThreadedStreamReader(pin,inout);
                try{
                    perrread.start();
                    pinread.start();
                    if(verbose>2){
        ...
        Vector<HighScoringSegmentPair> hspvec;
        if (lowmem) {
            hspvec = HspReader.get(lowmem, tmpoutfile, new Vector<HighScoringSegmentPair>(), eval, pval, coverage, scval, ident, verbose,
                    nameshash, useallrounds);// here the blast output is filtered for valid hsp's
        } else {
            hspvec = HspReader.get(myblast, new Vector<HighScoringSegmentPair>(), eval, pval, coverage, scval, ident, verbose, nameshash,
        ...
        for(int i=0;i<arrsize;i++){
            retarr[i]=new Vector<HighScoringSegmentPair>();
        }
        int namenum=-1;
