...
public class HspReader {
    ...
    public static Vector<HighScoringSegmentPair> get(boolean lowmem, String filename, Vector<HighScoringSegmentPair> invec, double eval, double pval,
        ...
        return get(tmpstr.toString(), new Vector<HighScoringSegmentPair>(), eval, pval, coverage, scval, ident, verbose, nameshash, useallrounds);
    }//end get for lowmem

    public static Vector<HighScoringSegmentPair> get(String inreadstring, Vector<HighScoringSegmentPair> invec, double eval, double pval, float coverage,
        ...
        int qlength = -1;
        int qlengtheff = -1;
        HighScoringSegmentPair myhsp = new HighScoringSegmentPair();
        int index1;
                ...
                if (querydone == false) {
                    ...
                    if (readseq == false) {
                        ...
                        if ((inline.indexOf("</PRE>") > -1) || (inline.indexOf("</pre>") > -1)) {// if </PRE> is on the line /PRE for blast /pre for psyblast
                            ...
                            myhsp = new HighScoringSegmentPair();
        ...
        for (int i = 0; i < invec.size(); i++) {
            invec.setElementAt(makefinal((HighScoringSegmentPair) (invec.elementAt(i))), i);
            if (verbose > 3) {
                System.out.println("hit to:'"+((HighScoringSegmentPair) invec.elementAt(i)).hname+"'");
            }
        }// end for i
        for (int i = 0; i < invec.size(); i++) {
            myhsp = (HighScoringSegmentPair) invec.elementAt(i);
    ...
    //--------------------------------------------------------------------------
    static HighScoringSegmentPair makefinal(HighScoringSegmentPair inhsp) {
