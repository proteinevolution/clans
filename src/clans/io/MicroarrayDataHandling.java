...
public class MicroarrayDataHandling {
     ...
     * @return
     */
    public static int load(File infile, WindowAffyMicroarrayData parent) {
        try{
     ...
     * @return
     */
    public static float getglobalrelstdev(Vector<MicroarrayDatapoint[]> datvec) {
        int datnum=datvec.size();
        float retval=0;
        float mystdev;
        float myavg,tmp;
        MicroarrayDatapoint[] datarr;
        int conditions;
        int subtract;
        for(int i=datnum;--i>=0;){
            datarr=(MicroarrayDatapoint[])datvec.get(i);
            myavg=0;
            mystdev=0;
     ...
     * @return
     */
    public static Vector<MicroarrayDatapoint[]> getfoldchange(Vector<MicroarrayDatapoint[]> datvec, boolean useavg){
        int datnum=datvec.size();
        MicroarrayDatapoint[] tmparr;
        int arrsize;
        if(useavg){
            int valint;
            //the amount of data doesn't change
            for(int i=0;i<datnum;i++){
                tmparr=(MicroarrayDatapoint[])datvec.elementAt(i);
     ...
     * @return
     */
    public static Vector<MicroarrayDatapoint[]> readdata(Vector<Replicates> datavec) {
        int datnum=datavec.size();
        File tmpfile;
        Replicates myreplicate;
        HashMap<String, Object> nameshash = new HashMap<String, Object>();
        Vector<MicroarrayDatapoint[]> temp_datapoints = new Vector<MicroarrayDatapoint[]>();
        Vector<indata> temp_indata = new Vector<indata>();
        indata tmpdat;
        MicroarrayDatapoint mypoint;
        double sumval,sumsqval;
        MicroarrayDatapoint[] tmparr;
        Vector<MicroarrayDatapoint[]> retvec=new Vector<MicroarrayDatapoint[]>();
        int unknownval=0;
        int presentval=1;
        int absentval=-1;
        ...
        for(int i=0;i<datnum;i++){
            ...
            for(int j=0;j<myreplicate.replicates;j++){
                ...
                for(int k=temp_indata.size();--k>=0;){
                    ...
                    if(nameshash.containsKey(tmpdat.name)){
                        ...
                        mypoint=((MicroarrayDatapoint)nameshash.get(tmpdat.name));
                        ...
                        mypoint=new MicroarrayDatapoint();
            ...
            tmparr=new MicroarrayDatapoint[namesnum];
            for(int j=namesnum;--j>=0;){
                mypoint=(MicroarrayDatapoint)nameshash.get(names[j]);
                sumval=0;
                sumsqval=0;
        ...
        MicroarrayDatapoint[] tmparr2;
        nameshash.clear();
        for(int i=0;i<retvec.size();i++){
            tmparr=(MicroarrayDatapoint[])retvec.elementAt(i);
            for(int j=tmparr.length;--j>=0;){
                if(nameshash.containsKey(tmparr[j].name)){
                    tmparr2=(MicroarrayDatapoint[])nameshash.get(tmparr[j].name);
                    tmparr2[i]=tmparr[j];
                }else{
                    tmparr2=new MicroarrayDatapoint[retvec.size()];
        ...
        temp_datapoints = new Vector<MicroarrayDatapoint[]>();
        String[] names=(String[])(nameshash.keySet().toArray(new String[0]));
        java.util.Arrays.sort(names);
        for(int i=names.length;--i>=0;){
            temp_datapoints.addElement((MicroarrayDatapoint[]) nameshash.get(names[i]));
        }
