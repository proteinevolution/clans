...
public class findclusters {
    ...
    public static Vector <cluster> getlinkage(minattvals[] attvals, int minlinks, int minseqnum, int seqnum) {
        //form clusters with at least each seq connected by minlinks to the cluster
    ...
    //--------------------------------------------------------------------------
    static void multilinkage(minattvals[] attvals, Vector <cluster>retvec, int minseqnum, int minlinks, int seqnum) {
        //get clusters of linkage N
        //quite a bit more complex than single linkage
        /*take the sequence with the highest number of connections.
         *from those connected to sequence0 select the N-1 with highest numbers of connections.
         *see which sequences they ALL find. If none, change in turn all seqs selected in step 2.
         *once a cluster "seed" of N-sequences is present, just add sequences connected with more than N
         *continue until no 1st step sequences have more than "N" connections
         */
        ...
        for (int i = 0; i < seqnum; i++) {
            ...
            tmparr = new int[3];
            tmparr[0] = -1;//groupvals; is a sequence assigned to a cluster and if so, which cluster
            //groupval: -3=used as seed2 in last round, -2=don't use as seed, -1=unassigned, 0=added to this cluster; >0=number of last cluster this seq was assigned to
            tmparr[1] = 0;//used as seqshits (flag whether or not this sequence is relevant in this calculation
            tmparr[2] = 0;//how many connections each sequence has to all others
            clusterhash.put(hashkeys[i], tmparr);//groupvals,
        }//end for i
        getmaxnum(attvals, clusterhash, hashkeys, 2);//get the maximum number of connections for each sequence
        //for(int i=0;i<seqnum;i++){
        ...
        while (done == false) {
            ...
            maxconn = 0;
            for (int i = 0; i < seqnum; i++) {
                if (((int[]) clusterhash.get(hashkeys[i]))[0] == 0 || ((int[]) clusterhash.get(hashkeys[i]))[0] == -3) {
                    ((int[]) clusterhash.get(hashkeys[i]))[0] = -1;
                }
                if (((int[]) clusterhash.get(hashkeys[i]))[0] == -1 && ((int[]) clusterhash.get(hashkeys[i]))[2] > maxconn) {//if this sequence is unassigned and has max connections
            ...
            if (java.lang.reflect.Array.getLength(conarr) < minlinks) {//if I found no new sequence with at least minlinks connections (no new clusters possible)
                done = true;
                break;
            }
            for (int i = 0; i < seqnum; i++) {
                ((int[]) clusterhash.get(hashkeys[i]))[1] = 0;//seqshits[i]=0;//the number of links connecting sequences to this group
            }//end for i
            ...
            while (java.lang.reflect.Array.getLength(newcluster.member) == 0) {
                //get the minlinks other sequences that share the most hits with this sequence
                tmpseqs = getmaxshared(attvals, clusterhash, hashkeys, maxnum, minlinks);
                //now see that I found at least minlinks valid connected sequences
                for (int i = 0; i <= minlinks; i++) {
                    if (tmpseqs[i] == -1) {
                        //remove the current seed from analysis until end (where it might come back in)
                        ((int[]) clusterhash.get(hashkeys[maxnum]))[0] = -2;//skip this seq for the rest of the analysis
                        continue GETCLUSTER;
                    }
                }//end for i
                for (int i = java.lang.reflect.Array.getLength(tmpseqs) - 1; i >= 0; i--) {
                    ((int[]) clusterhash.get(hashkeys[tmpseqs[i]]))[0] = -3;//groupvals[tmpseqs[i]]=-3;//am checking, remeber it is used as seed2
                }//end for i
                //now I have the N most connected sequences sharing the most connections and directly connected
                //next, see if I can start a cluster (i.e. if there are any sequence found by all of them)

                tmpseqs2 = getfound(attvals, clusterhash, hashkeys, tmpseqs, minlinks);//get all sequences found by all of the seqs in tmpseqs
                ...
                for (int i = java.lang.reflect.Array.getLength(tmpseqs2) - 1; i >= 0; i--) {
                    ...
                    if (((int[]) clusterhash.get(hashkeys[tmpseqs2[i]]))[0] != -2) {
                        ((int[]) clusterhash.get(hashkeys[tmpseqs2[i]]))[0] = 0;//groupvals[i]=0;//added to cluster
                    }
                }//end for i
                //now see if I have members>minlinks
                int foundnew = 0;
                int newmembers;
                while (((newmembers = java.lang.reflect.Array.getLength(newcluster.member)) < minlinks) && (newmembers != foundnew)) {
                    //System.out.println("inwhile");
                    foundnew = newmembers;
                    //System.out.println("in while");
                    //if I don't, I need to re-check, including the new members until I do have more than minlinks
                    tmpseqs2 = getfound(attvals, clusterhash, hashkeys, newcluster.member, minlinks);
                    for (int i = java.lang.reflect.Array.getLength(tmpseqs2) - 1; i >= 0; i--) {
            ...
            //next, see which sequences they find with at least minlinks connections
            int lastmembers = 0;
            int newmembers;
            //System.out.println("startmembers="+java.lang.reflect.Array.getLength(newcluster.member));
            while ((newmembers = java.lang.reflect.Array.getLength(newcluster.member)) != lastmembers) {
    ...
    //--------------------------------------------------------------------------
    static int[] getfound(minattvals[] attvals, HashMap clusterhash, String[] hashkeys, int[] tmpseqs, int minlinks) {
    ...
    //--------------------------------------------------------------------------
    static int[] getmaxshared(minattvals[] attvals, HashMap clusterhash, String[] hashkeys, int maxnum, int minlinks) {
        ...
        for (int i = 1; i <= minlinks; i++) {
            retarr[i] = -1;
            maxconn[i] = minlinks - 1;//make sure I only get sequences with at least minlinks connections in the output
        }//end for i
        for (int i = 0; i < seqnum; i++) {
            if ((((int[]) clusterhash.get(hashkeys[i]))[1] == 1) && (((int[]) clusterhash.get(hashkeys[i]))[0] == -1)) {//if this sequence had a connection to maxnum and is currently unassigned
    ...
    //--------------------------------------------------------------------------
    static int[] getconnecteds(int maxnum, HashMap clusterhash, String[] hashkeys, minattvals[] attvals) {
        ...
        //now go through tmphash and get the integers of the sequences with connection to maxnum
    ...
    //--------------------------------------------------------------------------
    static void getmaxnum(minattvals[] attvals, HashMap clusterhash, String[] hashkeys, int hashpos) {
    ...
    //--------------------------------------------------------------------------
    //--------------------------------------------------------------------------
    static void singlelinkage(minattvals[] attvals, Vector <cluster> retvec, int minseqnum, int seqnum) {
        ...
        while (foundnew) {
            ...
            if (newnum > -1) {
                System.out.print(".");
                tracesingle(attvals, seqnum, groupvals, counter, newnum, attnum);//trace the single linkages
                counter++;
            }//end if newnum>-1
        }//end while foundnew
    ...
    //--------------------------------------------------------------------------
    static void tracesingle(minattvals[] attvals, int seqnum, int[] groupvals, int counter, int newnum, int attnum) {
        //assign all single linked sequences the same counter value in groupvals.
        groupvals[newnum] = counter;
        for (int i = 0; i < attnum; i++) {
            if ((attvals[i].query == newnum) && (groupvals[attvals[i].hit] != counter)) {
                tracesingle(attvals, seqnum, groupvals, counter, attvals[i].hit, attnum);
            } else if ((attvals[i].hit == newnum) && (groupvals[attvals[i].query] != counter)) {
                tracesingle(attvals, seqnum, groupvals, counter, attvals[i].query, attnum);
            }
        }//end for i
        return;
    }//end tracesingle
    ...
    //---------------------convex clustering------------------------------------
    //--------------------------------------------------------------------------
    public static Vector <cluster> getconvex(minattvals[] attvals, float sigmafac, int minseqnum, int seqnum) {
        //return a vector containing all clusters found by a polythenic additive method
        //assign all sequencs to one cluster. then take always the sequence with highest overall attraction and seed it to a new cluster
        //next add all sequences with attractions higher to the new cluster than average+factor*variance
        ...
        //then take a node from this base cluster and add all those with higher affinity to it than to the base
        Vector currvec = new Vector(seqnum);
        cluster currcluster;
        float avgatt = getavgatt(basevec, attvals, hashkeys);//get the average attraction for the values in that vector
        float varatt = getvaratt(basevec, attvals, avgatt, hashkeys);//get the variance of attraction values
        int seed;
        while (basevec.size() > 0) {
            //System.out.println("doing getmaxatt");
            seed = getmaxatt(basevec, attvals, hashkeys);//get the vector element with the highest overall attraction
            //System.out.println("done getmaxatt");
            //System.out.println("seed="+seed+" basevec.size="+basevec.size());
            if (seed == -1) {//if I have no further attraction values in the matrix
            ...
            getcluster(seed, basevec, attvals, currvec, avgatt, varatt, sigmafac, hashkeys);//get one more cluster
            //System.out.println("done getcluster");
            currcluster = new cluster();
            currcluster.member = new int[currvec.size()];
            for (int i = currvec.size() - 1; i >= 0; i--) {
                currcluster.member[i] = ((Integer) currvec.elementAt(i)).intValue();
            }//end for i
        ...
        retvec.clear();
        //now add them back in sorted order and filter for minimum sequence number
    ...
    //get the average attraction between these two clusters
    int seqnum1=java.lang.reflect.Array.getLength(c1.member);
    int seqnum2=java.lang.reflect.Array.getLength(c2.member);
    float retval=0;
    float skipped=0;//used in the bootstrapping procedure where some values are changed to -1 (removed)
    for(int i=0;i<seqnum1;i++){
    for(int j=0;j<seqnum2;j++){
    if(attvals[c1.member[i]][c2.member[j]]>=0){
    retval+=attvals[c1.member[i]][c2.member[j]];
    }else{
    skipped++;
    }
    }//end for j
    }//end for i
    return retval/((seqnum1*seqnum2)-skipped);
    }//end getavgatt*/    //--------------------------------------------------------------------------
    static int getmaxatt(Vector invec, minattvals[] attvals, String[] hashkeys) {
        ...
        for (int i = 0; i < elements; i++) {
            tmphash.put(hashkeys[((Integer) invec.elementAt(i)).intValue()], new Integer(i));
            sumvals[i] = 0;
        }//end for i
        float maxval = 0;
        int maxnum = -1;
        ...
        for (int i = 0; i < attnum; i++) {
            if (tmphash.containsKey(hashkeys[attvals[i].query])) {
                sumvals[((Integer) tmphash.get(hashkeys[attvals[i].query])).intValue()] += attvals[i].att;
            }
            if (tmphash.containsKey(hashkeys[attvals[i].hit])) {
                sumvals[((Integer) tmphash.get(hashkeys[attvals[i].hit])).intValue()] += attvals[i].att;
            }
        }//end for i
        for (int i = 0; i < elements; i++) {
            if (sumvals[i] > maxval) {
    ...
    //--------------------------------------------------------------------------
    static void getcluster(int seed, Vector basevec, minattvals[] attvals, Vector currvec, float avgatt, float varatt, float sigmafac, String[] hashkeys) {
        ...
        //now add all those values with attraction to currvec greater than to basevec.
        ...
        while (foundnew == true) {
            ...
            for (int i = 0; i < elements; i++) {
                curratt = getavgatt(((Integer) basevec.elementAt(i)).intValue(), currvec, attvals, hashkeys, tmphash);
            ...
            if (maxnum > -1) {
                if (limit < maxatt) {
                    tmphash.put(hashkeys[((Integer) basevec.elementAt(maxnum)).intValue()], null);
    ...
    //--------------------------------------------------------------------------
    static float getavgatt(int newpos, Vector newgroup, minattvals[] attvals, String[] hashkeys, HashMap tmphash) {
        ...
        for (int i = 0; i < attnum; i++) {
            if (attvals[i].hit == newpos && tmphash.containsKey(hashkeys[attvals[i].query])) {
                if (attvals[i].att >= 0) {
                    retval += attvals[i].att;
                } else {
                    skipped++;
                }
            } else if (attvals[i].query == newpos && tmphash.containsKey(hashkeys[attvals[i].hit])) {
                if (attvals[i].att >= 0) {
                    retval += attvals[i].att;
                } else {
                    skipped++;
    ...
    static float getavgatt(Vector invec, minattvals[] attvals, String[] hashkeys) {
        ...
        //note, the attraction values should be symmetrical and only those >=0 should be considered
        int elements = invec.size();
        HashMap tmphash = new HashMap(elements);
        for (int i = 0; i < elements; i++) {
            tmphash.put(hashkeys[((Integer) invec.elementAt(i)).intValue()], null);
        }//end for i
        int attnum = java.lang.reflect.Array.getLength(attvals);
        float sumval = 0;
        int skipped = 0;
        for (int i = 0; i < attnum; i++) {
            if (tmphash.containsKey(hashkeys[attvals[i].query]) || tmphash.containsKey(hashkeys[attvals[i].hit])) {
                if (attvals[i].att >= 0) {
    ...
    //--------------------------------------------------------------------------
    static float getvaratt(Vector invec, minattvals[] attvals, float avgval, String[] hashkeys) {
    ...
    static Vector <cluster> getnetwork(minattvals[] attvals, int minseqnum, boolean dooffset, boolean globalaverage, int seqnum, int maxrounds) {
        ...
        //all of the links are fully described in the minattvals array (i.e. hit, query and value)
        ...
        for (int i = java.lang.reflect.Array.getLength(attvals); --i >= 0;) {
            ...
            if (elements[tmpdat.hit].links == null) {
                elements[tmpdat.hit].links = new ArrayList();
                elements[tmpdat.hit].weights = new ArrayList();
            }
            elements[tmpdat.hit].links.add(elements[tmpdat.query]);
            elements[tmpdat.hit].weights.add(new Float(tmpdat.att));
            avgval += tmpdat.att;
        }//end for i
        //now I have the thing in an arraylist of element objects with directional links
        if (dooffset) {
            //if I want to offset the values by their average value or the global average value
            if (globalaverage) {
            ...
            for (int i = seqnum; --i >= 0;) {
                ...
                if (myelem.weights != null) {
                    for (int j = myelem.weights.size(); --j >= 0;) {
                        myelem.weights.set(j, new Float(myelem.weights.get(j).floatValue() - avgval));
                    }//end for i
                }
            }//end for i
        }//end if dooffset
        ...
        HashMap<Integer, ArrayList> tmphash = new HashMap();
        Integer currid;
        ...
        System.out.println("sorting clusters by size (" + java.lang.reflect.Array.getLength(clusters) + ")");
    ...
    public static void cluster(element[] elements, int maxrounds) {
        ...
        int myid, maxid = -1;
        float maxval = 0;
        HashMap<Integer, Float> assignhash = new HashMap();
        ...
        while (changed && currround <= maxrounds) {
            ...
            for (int i = java.lang.reflect.Array.getLength(elements); --i >= 0;) {
                ...
                //now loop through the emission values and get the lowest highest emission id
                assignhash.clear();
                if (myelement.links != null) {
                    ...
                    for (int j = myelement.links.size(); --j >= 0;) {
                        ...
                        currweight = new Float(myelement.weights.get(j).floatValue());//make a copy of this as I might otherwise be actually changing the value of this weight!
                        //System.out.println("\tlink " + j + " weight=" + currweight + " -->id:" + currid);
                        if (assignhash.containsKey(currid)) {
                            currweight += assignhash.get(currid);
                        }
                        assignhash.put(currid, currweight);
                    }//end for j
                    //now all I have to do is find the assigned id with the highest sum value
                    ...
                    //System.out.println("\tmaxid=" + maxid + " maxweight=" + maxval);
            ...
            //now for the difficult part. if I just assign things this way, elements just continually swap assignments.
            //I need to take that into account with some sort of reverse lookup...
            for (int i = java.lang.reflect.Array.getLength(elements); --i >= 0;) {
                myelement = elements[i];
                //System.err.println(i + "\tid:" + myelement.id + "(" + myelement.weight + ")\tnewid:" + myelement.newid + "(" + myelement.newweight + ")");
                myid = myelement.id;
                //now loop through the linked objects and see whether any of them would change to this id
                //if they do, subtract their weight from the current assignment weight.
                //if the new assignment weight drops below the old one, don't change the assignment!
                //NOTE: only do this if the new assignment id is larger than the old one! (for directionality and controling forever loops)
                //System.out.println(i + " " + myelement.name + " currid=" + myid + "/" + myelement.weight + " newid=" + myelement.newid + "/" + myelement.newweight);
                if (myelement.newid > myid) {
                    //System.out.println("\t newid > old id");
                    for (int j = myelement.links.size(); --j >= 0;) {
                        if (myelement.links.get(j).newid == myid) {
                            //System.out.println("\t\tID swap between " + j + " and " + i + " weight=" + myelement.weights.get(j).floatValue());
                            //then subtract this weight from the newid weight
                            if ((myelement.weights.get(j)).floatValue() > 0) {//if this was a positive interaction leading to the clustering
                                myelement.newweight -= (myelement.weights.get(j)).floatValue();
                            }//else don't do anything
                        }
                    }//end for j
                }
                //System.out.println(i + " " + myelement.name + " id:" + myelement.id + "(" + myelement.weight + ")\tnewid:" + myelement.newid + "(" + myelement.newweight + ")");
                if (myelement.newweight > 0 && myelement.newweight > myelement.weight) {
                    ...
                    //System.out.println("\tNEW ASSIGNMENT: id=" + myelement.newid + " weight=" + myelement.newweight);
        ...
        if (changed == false) {
            System.out.println("NO more chages performed after iteration :" + currround);
        } else {
            System.out.println("Stopped clustering because the limit ot iterations was reached: " + currround);
        }
    //that's it. the elements should have their new id assigned!
    }//end cluster

    /*static Vector getnetwork(minattvals[] attvals, int minseqnum, boolean dooffset, boolean globalaverage, int seqnum, int maxrounds) {
    //use a "network approach
    //each element is assigned  an input value
    //it emits that value to the next layer which sums the emissions of the same value
    //in the next round each node emits the value of the highest sum it encountered
    //repeat until no more changes
    int attnum = java.lang.reflect.Array.getLength(attvals);
    netnode[] nodes = new netnode[seqnum];
    for (int i = 0; i < seqnum; i++) {
    nodes[i] = new netnode(i);
    }//end for i
    if (dooffset) {
    float average = 0;
    if (globalaverage) {
    //offset the values by the global average attraction value
    for (int i = 0; i < attnum; i++) {
    average += attvals[i].att;
    }//end for i
    average /= (seqnum * (seqnum - 1)) / 2;
    } else {
    //only get the average for the non-null attractions
    for (int i = 0; i < attnum; i++) {
    average += attvals[i].att;
    }//end for i
    average /= attnum;
    }
    //now assign the individual node objects their respective attractions
    for (int i = 0; i < attnum; i++) {
    nodes[attvals[i].query].addconn(attvals[i].hit, attvals[i].att - average);
    nodes[attvals[i].hit].addconn(attvals[i].query, attvals[i].att - average);
    }//end for i
    } else {
    //don't offset the values at all; just use as is.
    for (int i = 0; i < attnum; i++) {
    ...
    int round = 0;
    boolean changedval = true;
    int newnum;
    while ((changedval == true) && (round < maxrounds)) {//I don't want to do more than 100 rounds
    System.out.print("r" + round + ";");
    round++;
    changedval = false;
    for (int i = 0; i < seqnum; i++) {
    newnum = getmax(i, nodes);//get the maximum weight cluster for this node
    if (newnum != nodes[i].number) {
    changedval = true;
    if (newnum == nodes[i].oldnumber) {
    if (nodes[i].number < nodes[i].oldnumber) {
    //don't do anything; needed to keep nodes from swapping the same numbers over and over
    } else {
    nodes[i].oldnumber = nodes[i].number;
    nodes[i].number = newnum;
    }
    } else {
    nodes[i].oldnumber = nodes[i].number;
    nodes[i].number = newnum;
    ...
    }//end while changedval
    //now I should have all the groups assigned; now convert all the data into a vector of clusters and return
    ...
    cluster currcluster;
    while (changedval == true) {
    changedval = false;
    for (int i = 0; i < seqnum; i++) {
    if (changedval == false) {
    if (nodes[i].number != -1) {
    changedval = true;
    currnum = nodes[i].number;
    nodes[i].number = -1;
    if (tmpvec.size() > minseqnum) {
    currcluster = new cluster();
    currcluster.name = "cluster_" + counter;
    currcluster.member = new int[tmpvec.size()];
    ...
    static int getmax(int pos, netnode[] nodes) {
        ...
        float[][] retarr = (float[][]) tmphash.values().toArray(new float[0][2]);
