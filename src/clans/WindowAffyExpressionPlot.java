...
public class WindowAffyExpressionPlot extends javax.swing.JFrame {
    ...
    public WindowAffyExpressionPlot(ArrayList<MicroarrayDatapoint[]> inlist,float infloat,Vector<Replicates> datavec) {
        this.datlist=inlist;
    ...
    private void showselectedwindowmenuitemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_showselectedwindowmenuitemActionPerformed
        ...
        myselectwindow=new WindowAffySelectIdentifiers(this);
    ...
    ArrayList<MicroarrayDatapoint[]> datlist=null;
    int mousex=0;
    int mousey=0;
    int mousestartx=0;
    int mousestarty=0;
    ...
    WindowAffySelectIdentifiers myselectwindow=null;
        ...
        if(foldchangecheckbox.isSelected()){
            ...
            if(avgfoldchangecheckbox.isSelected()){
                ...
                for(int i=datlist.size();--i>=0;){
                    tmparr=(MicroarrayDatapoint[])datlist.get(i);
            ...
            for(int i=0;i<datnum;i++){
                if(drawdat[i]==1){
                    ...
                    if(shownamescheckbox.isSelected()){
                        ...
                        if(relexpcheckbox.isSelected()){
                            g.drawString(((MicroarrayDatapoint[])datlist.get(i))[0].name,(int)(xoffset+((conditions-1)*spacer)*xzoom),yoffset+(int)((drawheight-yoffset-yoffset)*(1-((tmparr[conditions-1].value-minmin)/(maxmax-minmin)))*yzoom)-yval);
                        }else{
                            ...
                            g.drawString("*"+((MicroarrayDatapoint[])datlist.get(i))[0].name,(int)(xoffset+((conditions-1)*spacer)*xzoom),yoffset+(int)((drawheight-yoffset-yoffset)*(1-((tmparr[conditions-1].value-maxmin[i][1])/(maxmin[i][0]-maxmin[i][1])))*yzoom)-yval);
                        }
                    }
                }
